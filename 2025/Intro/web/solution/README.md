...
The main section of the file `index.html` contains JavaScript code. A portion of that code is obfuscated.
...
A careful reader will notice the `// draw()` function is commented out.
To run the solution:

1. Remove the comment markers to restore the function (change `// draw()` to `draw()`).
2. Open `index.html` in a web browser.

That's it! The page will execute the drawing routine:
